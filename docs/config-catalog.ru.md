<!-- Сгенерировано скриптом scripts/gen-config-catalog.ts — не редактируйте вручную.
     Для повторной генерации выполните `pnpm run gen-config-catalog`. -->

# Каталог конфигураций плагинов

Каждый блок `config:`, который может задать запись `cordis.yml`: для каждого загружаемого пакета harness — дословная декларация конфигурации (вместе с JSDoc), которую получает его функция `apply` или конструктор сервиса; каждый упомянутый в ней тип вставлен рядом (типы, локальные для пакета) или дан ссылкой (все остальные). Вставка — это полный объявленный тип конфигурации плагина: поле, которое рантайм-схема сознательно исключает, является runtime-only seam (об этом говорит его собственный JSDoc) и не задаётся из `cordis.yml`. Это справочник по оси **deployment** — проводка, с которой работает автор плагина, находится в сгенерированном регионе Cordis API на каждой [странице подсистемы](subsystems/core.ru.md), схемы инструментов, обращённые к модели, — в [каталоге инструментов](tool-catalog.ru.md), а [subsystems/](subsystems/core.ru.md) документирует типы, на которые ссылаются эти декларации.

Этот файл СГЕНЕРИРОВАН из исходников (`scripts/gen-config-catalog.ts`) и проверяется на актуальность командой `pnpm run verify-config-catalog` (часть `doc-sync`) — не редактируйте его вручную. Блоки деклараций оформлены блоком кода с меткой `ts config-catalog` (doc-typecheck его пропускает: одиночная декларация со ссылками на импорты не компилируется сама по себе). Генератор также сверяет рантайм-схему schemastery со вставленной декларацией — каждый ключ, проверяемый схемой, включая вложенные, должен отыскиваться в объявленном типе конфигурации, — так что вставка не может скрыть поле, принимаемое загрузчиком.

Строка `Requires:` перечисляет ключи сервисов, которые плагин `inject`-ит: дерево `cordis.yml` должно также загружать провайдеров этих сервисов. Охват — ярус harness (`packages/`); vendored-плагины cordis, которые дерево конфигурации может подгружать дополнительно (`hmr`, консольный логгер, …), представляют собой закреплённый апстрим-код ([политика вендоринга](../vendor/README.md)) и здесь не каталогизируются.
...
Depends on: [`AgentOptions`](subsystems/core.ru.md) · [`SessionId`](subsystems/core.ru.md)
...
Depends on: [`ToolPresentationMode`](subsystems/tools.ru.md)
...
Source: [`packages/llm/llm-key-rotation/src/config.ts:34`](../packages/llm/llm-key-rotation/src/config.ts)
...
Depends on: [`ApprovalPolicy`](subsystems/approval.ru.md) · [`SandboxMode`](subsystems/sandbox.ru.md)
...
Depends on: [`SandboxMode`](subsystems/sandbox.ru.md)
...
Depends on: [`AgentOptions`](subsystems/core.ru.md)
...
Depends on: [`SubagentReportDelivery`](subsystems/subagent.ru.md)
...
## Загружаемые плагины без конфигурации

Эти плагины загружаются из записи `cordis.yml` без блока `config:`; API конфигурации они не объявляют.

- `@deepseek-ai/dsh-agent` ([`packages/core/agent/src/index.ts`](../packages/core/agent/src/index.ts))
- `@deepseek-ai/dsh-api-gateway` — требует `typert` ([`packages/api/gateway/src/index.ts`](../packages/api/gateway/src/index.ts))
- `@deepseek-ai/dsh-api-remotes` ([`packages/api/remotes/src/index.ts`](../packages/api/remotes/src/index.ts))
- `@deepseek-ai/dsh-authorization` — требует `credentials` ([`packages/credentials/authorization/src/index.ts`](../packages/credentials/authorization/src/index.ts))
- `@deepseek-ai/dsh-client-locale` ([`packages/client/locale/src/index.ts`](../packages/client/locale/src/index.ts))
- `@deepseek-ai/dsh-client-modules` — требует `webServer` · `loader` ([`packages/client/modules/src/index.ts`](../packages/client/modules/src/index.ts))
...
- `@deepseek-ai/dsh-client-ui-deliverables` — требует `systemPrompt` ([`packages/client/ui-deliverables/src/index.ts`](../packages/client/ui-deliverables/src/index.ts))
...
- `@deepseek-ai/dsh-command-compact` — требует `commands` · `compaction` ([`packages/compaction/command-compact/src/index.ts`](../packages/compaction/command-compact/src/index.ts))
- `@deepseek-ai/dsh-command-feedback` — требует `commands` ([`packages/feedback/command-feedback/src/index.ts`](../packages/feedback/command-feedback/src/index.ts))
- `@deepseek-ai/dsh-command-goal` — требует `commands` · `goals` ([`packages/goal/command-goal/src/index.ts`](../packages/goal/command-goal/src/index.ts))
- `@deepseek-ai/dsh-commands` ([`packages/interaction/commands/src/index.ts`](../packages/interaction/commands/src/index.ts))
- `@deepseek-ai/dsh-cordis-client-runner` ([`packages/extensions/cordis-client-runner/src/index.ts`](../packages/extensions/cordis-client-runner/src/index.ts))
- `@deepseek-ai/dsh-fs-e2b` — требует `e2b` ([`packages/e2b/fs-e2b/src/index.ts`](../packages/e2b/fs-e2b/src/index.ts))
- `@deepseek-ai/dsh-fs-observation-policy` ([`packages/fs/fs-observation-policy/src/index.ts`](../packages/fs/fs-observation-policy/src/index.ts))
- `@deepseek-ai/dsh-goal-round-driver` — требует `agents` · `goals` · `sessions` ([`packages/goal/goal-round-driver/src/index.ts`](../packages/goal/goal-round-driver/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker-auto` — требует `webServer` · `loader` ([`packages/host/directory-picker-auto/src/index.ts`](../packages/host/directory-picker-auto/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker-native` ([`packages/host/directory-picker-native/src/index.ts`](../packages/host/directory-picker-native/src/index.ts))
- `@deepseek-ai/dsh-host-plugin-inventory` — требует `loader` ([`packages/host/plugin-inventory/src/index.ts`](../packages/host/plugin-inventory/src/index.ts))
- `@deepseek-ai/dsh-llm` ([`packages/llm/llm/src/index.ts`](../packages/llm/llm/src/index.ts))
- `@deepseek-ai/dsh-lsp` ([`packages/lsp/lsp/src/index.ts`](../packages/lsp/lsp/src/index.ts))
- `@deepseek-ai/dsh-schedule` — требует `agents` · `sessions` · `tools` · `sessionPersistence` ([`packages/schedule/schedule/src/index.ts`](../packages/schedule/schedule/src/index.ts))
- `@deepseek-ai/dsh-session` ([`packages/core/session/src/index.ts`](../packages/core/session/src/index.ts))
- `@deepseek-ai/dsh-session-checkpoint-policy` — требует `llm` · `sessionPersistence` · `sessions` · `tools` ([`packages/session/session-checkpoint-policy/src/index.ts`](../packages/session/session-checkpoint-policy/src/index.ts))
- `@deepseek-ai/dsh-session-log-export` — требует `commands` ([`packages/session-query/session-log-export/src/index.ts`](../packages/session-query/session-log-export/src/index.ts))
- `@deepseek-ai/dsh-session-projection` ([`packages/session/session-projection/src/index.ts`](../packages/session/session-projection/src/index.ts))
- `@deepseek-ai/dsh-session-stats` — требует `sessionProjections` ([`packages/session/session-stats/src/index.ts`](../packages/session/session-stats/src/index.ts))
- `@deepseek-ai/dsh-skill-badge` — требует `skills` ([`packages/skill/skill-badge/src/index.ts`](../packages/skill/skill-badge/src/index.ts))
- `@deepseek-ai/dsh-storage` ([`packages/storage/storage/src/index.ts`](../packages/storage/storage/src/index.ts))
- `@deepseek-ai/dsh-subagent` ([`packages/subagent/subagent/src/index.ts`](../packages/subagent/subagent/src/index.ts))
- `@deepseek-ai/dsh-subprocess-local` ([`packages/subprocess/subprocess-local/src/index.ts`](../packages/subprocess/subprocess-local/src/index.ts))
- `@deepseek-ai/dsh-terminal` ([`packages/terminal/terminal/src/index.ts`](../packages/terminal/terminal/src/index.ts))
- `@deepseek-ai/dsh-tool-ask-user` — требует `tools` · `userQuestions` ([`packages/interaction/tool-ask-user/src/index.ts`](../packages/interaction/tool-ask-user/src/index.ts))
- `@deepseek-ai/dsh-tool-call-timeout-policy` — требует `tools` ([`packages/guard/timeout-policy/src/index.ts`](../packages/guard/timeout-policy/src/index.ts))
- `@deepseek-ai/dsh-tool-cordis` — требует `tools` · `systemPrompt` · `dynamicCordisRunner` · `cordisInspect` ([`packages/extensions/tool-cordis/src/index.ts`](../packages/extensions/tool-cordis/src/index.ts))
- `@deepseek-ai/dsh-tool-subagent-control` — требует `tools` · `subagents` ([`packages/subagent/tool-subagent-control/src/index.ts`](../packages/subagent/tool-subagent-control/src/index.ts))
- `@deepseek-ai/dsh-user-questions` ([`packages/interaction/user-questions/src/index.ts`](../packages/interaction/user-questions/src/index.ts))
- `@deepseek-ai/dsh-workspace` — требует `storageDomain` · `sessionPersistence` ([`packages/workspace/workspace/src/index.ts`](../packages/workspace/workspace/src/index.ts))

## Пакеты capability seam (напрямую не загружаются)

Абстрактные классы сервисов — вместо них развёртывание загружает пакет с конкретной реализацией ([capability seams](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)).

- `@deepseek-ai/dsh-attachment` — абстрактный `AttachmentStore` ([`packages/attachment/attachment/src/index.ts`](../packages/attachment/attachment/src/index.ts))
- `@deepseek-ai/dsh-code-runtime` — абстрактный `CodeRuntime` ([`packages/code-runtime/code-runtime/src/index.ts`](../packages/code-runtime/code-runtime/src/index.ts))
- `@deepseek-ai/dsh-compaction` — абстрактный `CompactionEngine` ([`packages/compaction/compaction/src/index.ts`](../packages/compaction/compaction/src/index.ts))
- `@deepseek-ai/dsh-credentials` — абстрактный `CredentialProvider` ([`packages/credentials/credentials/src/index.ts`](../packages/credentials/credentials/src/index.ts))
- `@deepseek-ai/dsh-file-reference` — абстрактный `FileReferenceService` ([`packages/context/file-reference/src/index.ts`](../packages/context/file-reference/src/index.ts))
- `@deepseek-ai/dsh-fs` — абстрактный `FileSystem` ([`packages/fs/fs/src/index.ts`](../packages/fs/fs/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker` — абстрактный `DirectoryPicker` ([`packages/host/directory-picker/src/index.ts`](../packages/host/directory-picker/src/index.ts))
- `@deepseek-ai/dsh-jobs` — абстрактный `JobRegistry` ([`packages/jobs/jobs/src/index.ts`](../packages/jobs/jobs/src/index.ts))
- `@deepseek-ai/dsh-sandbox` — абстрактный `SandboxProvider` ([`packages/sandbox/sandbox/src/index.ts`](../packages/sandbox/sandbox/src/index.ts))
- `@deepseek-ai/dsh-session-persistence` — абстрактный `SessionPersistence` ([`packages/session/session-persistence/src/index.ts`](../packages/session/session-persistence/src/index.ts))
- `@deepseek-ai/dsh-session-query` — абстрактный `SessionQueryEngine` ([`packages/session-query/session-query/src/index.ts`](../packages/session-query/session-query/src/index.ts))
- `@deepseek-ai/dsh-settings` — абстрактный `SettingsProvider` ([`packages/settings/settings/src/index.ts`](../packages/settings/settings/src/index.ts))
- `@deepseek-ai/dsh-shell` — абстрактный `ShellExecutor` ([`packages/shell/shell/src/index.ts`](../packages/shell/shell/src/index.ts))
- `@deepseek-ai/dsh-spill` — абстрактный `SpillStore` ([`packages/spill/spill/src/index.ts`](../packages/spill/spill/src/index.ts))
- `@deepseek-ai/dsh-subprocess` — абстрактный `SubprocessRuntime` ([`packages/subprocess/subprocess/src/index.ts`](../packages/subprocess/subprocess/src/index.ts))
- `@deepseek-ai/dsh-workflow` — абстрактный `WorkflowEngine` ([`packages/workflow/workflow/src/index.ts`](../packages/workflow/workflow/src/index.ts))

## Библиотечные пакеты (без записи плагина)

Импортируются другими пакетами как библиотеки; `cordis.yml` не может их загрузить.
