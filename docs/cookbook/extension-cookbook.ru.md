...
Этот каскад — перестраиваемый слой политик. Используйте `ctx.tools.guard()`, когда инварианту нужен монотонный финальный запрет, `tools/execute`, когда плагин должен обернуть фактическое время жизни диспетчеризации (таймауты/повторы/метрики; заменяем только `exec.signal`), `tools/post-execute` для явного преобразования результата и `tools/result` для заключённого наблюдения неизменяемого финального исхода. Правило выбора даёт [руководство adding-a-tool](adding-a-tool.ru.md#политика-исполнения-и-наблюдение).

## Плагин UI

Плагин UI отображает из ленты `session/event` (поток токенов ассистента как `assistant/chunk`, плюс границы turn/step и активность инструментов) и заводит ввод обратно через `agent.followup()` / `agent.steer()`. Браузерный плагин, добавляющий бизнес-строку во встроенный Web Client, вместо этого регистрирует `ConversationNodeDefinition` и key-рендерер Chat; следуйте [руководству Conversation Node](adding-a-conversation-node.md).
...
*Драйвер протокола* адаптирует участника проводного обмена к `ctx.agents`; он может обслуживать UI или клиента автоматизации. stdio-драйвер владеет stdout, создаёт или возобновляет агентов через фабрику и отображает запросы протокола в `followup()` или `cancel()`. Низкоуровневый запрос промпта возвращает свою долговечную квитанцию постановки в очередь; результат он не добывает сопоставлением `MessageId` с `turn/end`. Статус агента целиком публикуйте отдельно. Метод автоматизации может ждать от своей квитанции до следующего простоя и резюмировать именно этот явно выделенный интервал, тогда как UI обычно продолжает наблюдать открытый поток событий. Разрушайте агентов через `AgentHandle.dispose()`, чтобы освобождение ресурсов достигало quiescence.

[`packages/acp/acp`](../../packages/acp/acp) — проработанный пример только для автоматизации: он выставляет свежие текстовые сессии поверх Agent Client Protocol JSON-RPC stdio, выдаёт зафиксированный текст ассистента и регистрирует одноразовый машинный ответчик разрешений для агентов, которыми владеет. Его [README](../../packages/acp/acp/README.md) определяет точные методы, порядок событий и контракт жизненного цикла.
...
|---|---|
| Система хуков (уровень пользователя + проекта) | слушатели на `agent/session-start`, `agent/pre-step`, `agent/request`, `tools/pre-execute`, `tools/post-execute` и `agent/turn-stopping`; каскады возвращают типизированные решения, при этом `agent/turn-stopping` может направить другой ход; мосты `dsh-hooks-claude-code` / `dsh-hooks-codex` отображают конфигурационные файлы хуков на эти точки расширения |
| `/goal` | `ctx.goals` владеет долговечным состоянием, `dsh-goal-round-driver` планирует раунды в одной сессии через публичный `Agent`, а отдельные производители команд/инструментов дают управление человеку/модели |
| `/loop` | по событию сессии `turn/end` — `followup()` следующей итерации; либо принудительное продолжение |
| Динамический воркфлоу | `ctx.workflowEngine` + движок worker-thread + инструмент `workflow`; структурные внутрипроцессные потомки принуждают вывод с помощью scoped-регистраций промптов/инструментов, монотонного guard инструмента, финального коммита `tools/result` (включая объемлющий `run_code`) и монотонного маркера `concludeTurn()` исполнения структурированного вывода |
| Сообщения в очереди + steering | базовые `Agent.followup()` / `Agent.steer()` |
| Компакция контекста (авто + ручная) | seam `ctx.compaction` + `dsh-compaction-basic`; автоматическое давление работает на последовательных `agent/pre-step`, каноническое восстановление после переполнения — на `agent/request-error`, а ручные вызывающие пользуются тем же сервисом компакции ([Agent Note о компакции](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.md)) |
...
| Встроенные инструменты | `ctx.tools.register()`; схемы автоматически попадают в сборку — семейства `dsh-tool-*` (bash, fs, web, subagent, todo) — выпущенные примеры |
...
| Дедлайн инструмента / повтор / метрики | обёртывание базовой диспетчеризации через `tools/execute`; обёртка может заменить `exec.signal`, делегировать и осмотреть нормализованный результат в одном лексическом времени жизни |
...
| Plan mode | [`@deepseek-ai/dsh-plan-mode`](../../packages/plan/plan-mode/README.md) — журналируемое состояние `plan/mode`, направляющая секция `plan:policy`, вход `/plan [message]`, прямой выход `/plan off` и проверяемый пользователем выход `exit_plan_mode`; принуждение остаётся на независимых осях песочницы/разрешений |
...
| Бизнес-узел Chat Web Client'а | регистрация `ConversationNodeDefinition` и key-рендерера `conversation.chat.node` |
| SessionTelemetryBackend / воспроизводимый трассировочный след | `session/event` → JSONL; воспроизведение = `sessions.create(id, { seed })` |
