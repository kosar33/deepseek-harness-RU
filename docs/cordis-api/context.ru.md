...
Контекст — центральный объект Cordis: каждый сервис, каждое событие и каждый API жизненного цикла достигается через `ctx`. Методы событий описаны в разделе [События](events.ru.md), эффекты и текущий fiber — в разделе [Fiber](fiber.ru.md), загрузка плагинов — в разделе [Реестр](registry.ru.md).
...
Ключ-symbol, под которым disposer раскрывает своё диагностическое дерево EffectMeta.
...
Регистрирует реализацию сервиса, принадлежащую текущему fiber.
