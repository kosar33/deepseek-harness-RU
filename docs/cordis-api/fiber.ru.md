...
Fiber — один загруженный экземпляр плагина: его состояние жизненного цикла, валидированная конфигурация и зарегистрированные эффекты. `ctx.fiber` — текущий fiber, а `ctx.effect()` делегирует ему.
...
Экземпляр рантайма одного применения плагина.
...
Снапшот реализаций требуемых сервисов, пока плагин загружен; иначе `undefined`.
...
Текущий переход загрузки/выгрузки, если такой сейчас выполняется.
...
Ничего не возвращает, пока fiber активен.
...
- `config` — новая сырая конфигурация; валидируется до того, как что-либо перезапустится.
...
Либо одиночный disposer, либо промис такового, либо (возможно асинхронный) iterable, выдающий несколько — generator-эффекты регистрируют каждый выданный disposer по мере его появления.
...
Функция, возвращаемая эффектом для освобождения ресурсов при разборке.
...
Узел дерева, раскрывающий вложенные метки эффектов для диагностики.
