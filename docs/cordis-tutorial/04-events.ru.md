...
Слияние `interface Events` — событийный аналог слияния `interface Context` из главы 3: оно объявляет имя события и сигнатуру его слушателя, так что `ctx.emit` и `ctx.on` полностью типизированы. Конвенция именования `namespace/action` сохраняет плоское пространство имён событий читабельным.
...
`emit` — один из пяти режимов диспатча. Какой режим использует событие — часть его контракта: это решает, могут ли слушатели возвращать значения, работать конкурентно или замыкать друг друга накоротко:
...
Пропишите в `cordis.yml` только этот файл и запустите:
