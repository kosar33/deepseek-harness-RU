...
Каждая запись `cordis.yml` может нести блок `config`, а плагин объявляет схему, которая валидирует его до запуска `apply`. Плохая конфигурация срывает загрузку с точной ошибкой — плагин никогда не стартует наполовину сконфигурированным.
...
Теперь скормите ему нечто невалидное:
...
fiber плагина переходит в FAILED, а лаунчер этого туториала после печати ошибки завершается со статусом 1. Плагину также **СЛЕДУЕТ** отвергать конфигурацию, валидную по схеме, но называющую недоступный ресурс или провайдера, — как только он сможет разрешить эту ссылку.
