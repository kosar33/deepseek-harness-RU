...
Межпакетные зависимости пакетов harness `@deepseek-ai/dsh-*`, извлечённые из `peerDependencies` каждого пакета (канонический признак рантайм-зависимостей) и сгруппированные по иерархии `packages/<group>/<pkg>`. Ребро `a --> b` означает, что пакет `a` зависит от пакета `b`. У имён отброшен префикс `@deepseek-ai/dsh-`.
