...
Фреймворк Cordis и его базовые библиотеки вендорятся под [`vendor/`](../vendor/README.md) и публикуются в скоупе `@deepseek-ai`, потому что каждый пакет harness объявляет фреймворк peer-зависимостью: публикация harness публикует вместе с ним и этот слой, а под апстримными именами такая публикация захватила бы их на реестре. Эта страница — отображение имён; решение и его последствия описаны в [Agent Note о рескоупе](../.agents/notes/implemented/process/2026-08-10-vendor-package-rescope.md), апстримные коммиты — в [`vendor/README.md`](../vendor/README.md).
...
| `vendor/schemastery/` | `schemastery` | `@deepseek-ai/schemastery` | 3.18.0 | Схемы конфигураций (`Schema`) за `Config` каждого плагина |
...
| `vendor/timer/` | `@cordisjs/plugin-timer` | `@deepseek-ai/cordis-plugin-timer` | 1.1.2 | Таймеры на `ctx`, учитывающие освобождение ресурсов |
...
- **Имён каталогов и версий.** `vendor/hmr/` остаётся `vendor/hmr/`, и каждый пакет сохраняет апстримную версию, записанную в строке таблицы манифеста, поэтому вендоренное дерево по-прежнему читается как снапшот апстрима.
- **Диапазонов зависимостей.** Запись зависимости меняет ключ, но не диапазон: `"cordis": "^4.0.0-rc.7"` становится `"@deepseek-ai/cordis": "^4.0.0-rc.7"`. `linkWorkspacePackages` разрешает эти сохранённые диапазоны в закреплённые воркспейсы.
- **Встроенного префикса `cordis:` загрузчика.** `cordis:include` и `cordis:group` — протокольный префикс, не имя пакета.
...
- **Прозы вне `docs/`.** `vendor/*/README.md`, README пакетов и Agent Notes хранят имена, с которыми написаны; просто `cordis` там может быть ещё и именем опции Python SDK или id агентского пресета. Внутри `docs/` переименование применяется и к прозе, и к каждому блоку кода в Markdown.
...
[`scripts/rescope-vendor.ts`](../scripts/rescope-vendor.ts) отвечает за приведённое выше отображение и выполняет переименование — ни одну ссылку не переименовывают вручную:
...
Повторно применяйте его после апстримной синхронизации ([процедура](../vendor/README.md)), а затем выполните перегенерацию, которую он печатает: `pnpm install` для lockfile, `pnpm run gen-third-party-notices` и `pnpm run verify-translation-pairing --write` для затронутых двуязычных пар.
