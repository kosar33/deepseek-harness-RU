...
Одна страница на каждую подсистему DeepSeek Harness: что она такое, какими структурами данных оперирует и — там, где её обслуживают сервис `ctx` или область событий, — сгенерированный раздел **Cordis API** со справочником её сервисов и событий. Каталог дополняет [architecture.md](../architecture.ru.md), где описано *поведение* поперёк подсистем (карта сервисов, жизненный цикл сессия/ход/шаг, таксономия событий); каждая страница здесь — справочник словаря и связей одной подсистемы.

| Страница | Охватывает |
|---|---|
| [core.md](core.ru.md) | как `packages/core` управляет agent loop: пошаговое по пакетам описание цикла, создание агентов и владение (`AgentHandle`), контракты доставки/отмены/перехвата дескриптора `Agent` и общерепозиторные типовые паттерны (`…Map → производное объединение`, брендированные id) |
...
| [scope.md](scope.ru.md) | идентичность скоупированной регистрации, носители диспатча и принадлежащий системе контекст `Scope` |
...
| [goal.md](goal.ru.md) | персистентная идентичность цели (goal), снимки жизненного цикла, активация, записи изменений и атрибуция Round |
| [schedule.md](schedule.md) | записи напоминаний в пределах сессии, долговечные переходы, активные представления и доставка через обычный разговор |
...
| [session.md](session.ru.md) | полный каталог вариантов `SessionEventMap`, `TurnTrigger`/`TurnEndReason`, `deriveMessages()`, ограждение исполнения и отдельные события |
...
| [credentials.md](credentials.ru.md) | seam учётных данных: ссылки `CredentialRef` (никогда не значения) в конфигурации, разрешение на операцию, безопасный для UI `CredentialInfo`, слои источников провайдеров |
| [session-query.md](session-query.md) | логические записи, ограниченные точные чтения событий, трассировки отношений, семантические фильтры/документы и страницы полнотекстовых результатов |
...
| [session-title.md](session-title.ru.md) | долговечные снимки заголовков, цитируемые seq исходных сообщений и асинхронный контракт провайдера |
...
| [sandbox.md](sandbox.ru.md) | разрешение политики на сессию и seam ограничения процессов: режимы файловых эффектов, политики исполнения/провайдеров, `ConfinedArgv`, принуждение и fail-closed ошибки |
...
| [extensions.md](extensions.ru.md) | версионированные динамические плагины и пакеты Cordis, активация Host/Client, одобрение, инспекция рантайма и демонтаж жизненного цикла |
...
| [agent-team.md](agent-team.ru.md) | Агентские команды: неявная идентичность Lead, именованные continuable-участники, долговечный peer-почтовый ящик и общий DAG задач |
...
| [plan.md](plan.ru.md) | режим плана: только журналируемое состояние `plan/mode`, сброс ожидающего выбора, `PlanModeConfig`, арка рецензирования `exit_plan_mode` |
...
| [web-server.md](web-server.ru.md) | HTTP-носитель: `WebRouteKind`/`WebRoute`, порядок сопоставления, занимаемое место fallback, тапы индекса |
...
| [client-modules.md](client-modules.ru.md) | таблица веб-плагинов: объявления `dsh.client`, протокольная композиция `WebBootGraph`, маршрут бандла и тап индекса |
| [session-projection.md](session-projection.md) | seam проекции: `SessionProjectionMap`, чистая единица `ProjectionDefinition`, согласованный срез `ProjectionSnapshot`, лента изменений |
