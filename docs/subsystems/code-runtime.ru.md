...
Seam исполнения кода — [capability seam](../../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md), чьё Service Definition ([dsh-code-runtime](../../packages/code-runtime/code-runtime), `ctx.codeRuntime`) исполняет одну написанную моделью программу против предоставленных хостом асинхронных биндингов и сообщает, что она напечатала и вернула. Исполнение кода — **одна необязательная возможность**, а не часть стержня agent-loop, поэтому его словарь живёт здесь, а не в [core.ru.md](core.ru.md). Бэкенды различаются субстратом исполнения и исходным языком — оба это readonly-дескрипторы сервиса; Service Provider на worker-потоках и Consumer в реестре инструментов специфицируют [фундамент Code Mode](../../.agents/notes/implemented/feature/2026-06-15-code-mode.md) и [контракт типизированных возвратов](../../.agents/notes/implemented/feature/2026-07-20-code-mode-typed-tool-returns.md).
...
`CodeRunRequest` несёт **всё, на чём рантайм действует**, — согласно правилу «явное > неявное на границах пакетов» значения по умолчанию (бюджеты времени, лимиты вывода) — это валидируемая конфигурация реализации, а не скрытый `??` внутри `run()`:
...
Результат сообщает об ошибке как **поле**, а не отклонением `run()` — сообщать о неудавшейся программе — работа вызывающей стороны, а не путь исключения (совпадает с контрактом resolve-on-failure метода `ShellExecutor.run`):
...
Каждый `CodeBindingNamespace` становится одним глобальным объектом асинхронных вызываемых внутри программы (потребитель Code Mode передаёт один: `tools`). Аргументы и результаты должны быть lossless JSON и проходить без байтового лимита на уровне seam; рантайм может переносить их через structured clone. Пространство имён может объявить видимый программе класс ошибок, не заставляя рантайм знать имена потребителя: рантайм вводит настоящий конструктор и превращает отклонённые вызовы в его экземпляры. Рантайм также трактует имена биндингов как враждебный ввод (`__proto__` — обычное собственное свойство, а не коллизия прототипа):
...
Журналы — простые строки в порядке эмиссии. Рантайм захватывает вывод консоли и потоков программы, но метаданные каналов и методов консоли не входят в seam, потому что потребители рендерят только текст. Реализации ограничивают сериализованный внешний массив журналов плюс полезную нагрузку значения-завершения или сообщения о сбое; фиксированный синтаксис конверта результата и пробелы презентации потребителя не входят в этот учёт переменной полезной нагрузки. Переполнение — явный сбой, а не внутриполосная подстановка значения.
