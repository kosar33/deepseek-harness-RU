...
Сервис реестра человеческих команд из [`dsh-commands`](../../packages/interaction/commands). Интерактивные адаптеры используют его, чтобы обнаруживать и напрямую исполнять принадлежащие плагинам команды для точного агента, не создавая сообщение модели. Обоснованием диспатча и жизненного цикла владеет [Agent Note о командах](../../.agents/notes/implemented/feature/2026-07-19-plugin-command-registration.md); композицией и ограничениями — [README пакета](../../packages/interaction/commands/README.md).

Источник: [`packages/interaction/commands/src/index.ts`](../../packages/interaction/commands/src/index.ts)

## Метаданные ввода

Сервис выставляет один необязательный дескриптор неструктурированного ввода: подсказку плюс флаг приёма изображений. Доступность команд следует композиции плагинов: каждый адаптер, потребляющий реестр, видит каждое действующее определение.
...
Адаптер владеет отменой и передаёт точного целевого агента. `rawInput` начинается сразу после разобранного имени и сохраняет доставленный адаптером разделитель и суффикс. Результаты — прямые исходы UI, а не результаты инструментов или события сессии.
...
`sourceEventSeq` необязателен и допустим только при успехе. Когда присутствует, он называет более раннее событие, не являющееся командой, в журнале принимающей сессии; `command/done` сохраняет ту же ссылку, чтобы клиент мог объединить жизненный цикл команды с той доменной проекцией, не разбирая `text` и не полагаясь на соседние строки.

## Представления обнаружения и разбора

Адаптеры получают неизменяемые дескрипторы без обработчика после разрешения scope. `parseCommand()` возвращает `ParsedCommand` до разрешения в реестре; синтаксически корректный ввод всё ещё может называть недоступную команду.
