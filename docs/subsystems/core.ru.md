...
Подсистема **ядро** — это [`packages/core`](../../packages/core/README.md): пакеты, которые грузит любая композиция — event-sourced журнал сессии, сборка системного промпта, реестр инструментов, агентские типы и конкретный цикл, который ими управляет. Эта страница объясняет, что объявляет пара `agent`/`agent-loop` — как агент создаётся и кем владеется, а также контракты доставки, отмены и перехвата у дескриптора `Agent` — плюс два типовых паттерна, которые повторяет каждая подсистема. Страницы группы и остальное содержимое папки индексируются в [README подсистем](README.md).

## Стержень, пакет за пакетом

Ход проходит шесть пакетов одним циклом: драйвер в [`agent-loop`](../../packages/core/agent-loop) клеймит стоящий в очереди промпт, открывает ход в [журнале сессии](session.md) (`ctx.sessions`), собирает префикс запроса через [system-prompt](system-prompt.md) (`ctx.systemPrompt`) и выводит историю из журнала, передаёт ответ модели потоком через [seam потоковой передачи LLM](llm-streaming.md), диспетчеризует вызовы инструментов через [реестр инструментов](tools.md) (`ctx.tools`) и дописывает каждый видимый модели факт обратно в журнал прежде, чем следующий шаг выведет из него данные. Словарь разговора, который перемещает цикл, — `Message`, `ContentBlock`, `StreamChunk`, запрос к модели — объявлен в [`packages/llm`](../../packages/llm/README.md) и описан на [llm-streaming.md](llm-streaming.md).

| Пакет | Владеет | Страница |
|---|---|---|
| `session/` | Журнал `SessionEvent` с дописыванием и хранилище в памяти — единый источник истины (`ctx.sessions`) | [session.md](session.md) |
...
Потребители создают агентов через `ctx.agents` — `create()` строит свежие сессию и агента под одним заданным вызывающим `SessionId`, `resume()` сначала загружает персистированную сессию — либо декларативно через конфигурационные записи цикла. Программное создание возвращает дескриптор владельца:
...
`Agent` — поверхность, против которой программирует каждый плагин (UI, хуки, оркестраторы); `ctx.agents.get(id)` возвращает его, и [скоуп инициатора](#инициирующий-агент) несёт его. Конкретная реализация внутренняя для пакета dsh-agent-loop; вне цикла от неё никто не зависит. Единый метод `send` напрямую открывает маршрутизацию по цели и пробуждению; `followup`, `steer` и `inject` — его алиасы с фиксированным пресетом.
...
`running` описывает интервал, пока драйвер разбирает очередь целиком, и может охватывать несколько подряд идущих ходов из очереди; он не доказывает, что ход всё ещё открыт. Освобождение ресурсов удаляет агента из реестра и порождает `agent/disposed`; это не терминальное значение статуса. `followup()` не возвращает дескриптор: его `MessageId` идентифицирует факты долговременной вставки, клеймления и отбрасывания в почтовом ящике, а не последующий вывод ассистента или окончание хода. `whenIdle()` наблюдает весь агент целиком, поэтому промежуток от получения квитанции до простоя вправе называться запуском только тот, кто явно владеет этим промежутком ([decision](../../.agents/notes/implemented/architecture/2026-07-30-followup-enqueue-and-owned-runs.md)).
...
Для диспетчеризации после `agent/request` требуются `provider` и `model`. Присутствующий `maxTokens` должен быть положительным безопасным целым и ограничивает каждый запрос к разговорной модели; его отсутствие позволяет дефолту адаптера точной модели материализоваться до заголовка запроса либо иначе оставляет поведение провайдера неизменным. Секция промпта `deployment:persona` в скоупе агента может затмевать глобальную персону по умолчанию.

Почтовый ящик — словарь доставки: два упорядоченных списка ожидающих сообщений, которыми агент владеет как долговременной проекцией:
...
Каждое ожидающее появление — это его `UserMessage`; `MessageId` — единственная идентичность. `Inbox.append`, `prepend`, `replace`, `remove`, `clear`, `splice` и `claim` записывают нормализованные долговременные мутации `agent/inbox/spliced` и отвергают дублирующиеся ожидающие id. `replace(messageId, newMessage)` и `remove(messageId)` находят ожидающее сообщение в обоих списках; замена может сменить идентичность и порождает старое сообщение как отброшенное, а следом новое как вставленное. Обычные удаления и `clear()` — это отмены. `claim(target)` забирает предлагаемый батч шага — весь ввод `next-step` плюс, на границе хода, одно сообщение `next-turn` — сплайсами чистого удаления без порождения уведомлений об отбрасывании, а цикл отдельно порождает персональные уведомления claimed по каждому сообщению. Потребители всей очереди, например проекции UI, восстанавливают `nextTurn` и `nextStep` из долговременных сплайсов, тогда как потребители, следящие за одним сообщением, используют точные уведомления `agent/inbox/inserted`, `claimed` и `discarded`.
...
Причина — контролируемый TypeScript'ом внутрипроцессный ввод. Активный держатель отмены копирует её в рантайм-ориентированный `AbortSignal.reason`; сигнал не даёт кооперирующимся слушателям права классифицировать. Долговременный `turn/end` сохраняет грубый исход `{ kind: 'aborted' }`; запись того, кто запросил отмену, потребовала бы отдельного долговременного события, а не перегрузки терминального результата.

[Таксономия событий](../architecture.ru.md#события) владеет контрактами жизненного цикла `agent/*`, чекпоинтов и каскадов. Границы ходов и шагов — долговременные события сессии, а не порождения агента.
...
Решения перед шагом используют тот же тип `UserMessage` с идентичностью, что и долговременный ввод роли user. Вошедший батч авторитетен и сохраняет `id` и `source` каждого сообщения. Мосты хуков отображают свои нативные поля решений на этот типизированный результат.

Источник: [`packages/core/agent/src/types.ts`](../../packages/core/agent/src/types.ts)

`agent/pre-step` получает одну полезную нагрузку с эксклюзивным склеймленным батчем (`messages`), координатами предлагаемого шага (`turn`, `step`) и сигналом отмены текущего хода `signal`. Начальное предложение запускается внутри открытого хода до всякого шага; продолжение после инструмента может представить пустой склеймленный батч между шагами:

Он возвращает `PreStepDecision`. Reject не открывает шаг. Enter поставляет полный батч сообщений, дописываемый после `step/start`; склеймленные сообщения, опущенные финальным решением, остаются удалёнными, а ввод, вставленный после клеймления, остаётся ожидающим:
...
`agent/request-error` выполняется после закрытия неудавшегося шага модели и до закрытия его хода. Слушатели могут чинить долговременное состояние или выполнять работу политик, пока сигнал неудавшегося хода ещё жив. Обрабатывающий слушатель возвращает `{ kind: 'retry' }`, не вызывая `next()`; умолчательный `undefined` оставляет неудачу терминальной.
...
`agent/pre-step` — единственная серийная цепочка слушателей перед выводом запроса. `agent/turn-stopping` выполняется, когда у хода нет ни продолжения инструментом, ни свежего steering'а, перед одним финальным разбором накопившегося steering'а.

`agent/session-start` несёт `SessionStartSource` (почему начался жизненный цикл сессии; мост ключует свой матчер SessionStart по нему):
...
Точные условные поля конверта `SessionEvent`, двенадцать вариантов событий (`turn/start`, `turn/end`, `step/start`, `step/end`, `user/message`, `assistant/chunk`, `assistant/message`, `tool/call`, `tool/result`, `steering/message`, `todo/write`, `request/header`), правила проекции `deriveMessages()`, причины `TurnTrigger`/`TurnEndReason`, а также правила ограждения исполнения и автономных событий — на **[session.md](session.md)**. Как журнал становится долговременным — интерфейс `SessionPersistence`, бэкенды JSONL/SQLite, чекпоинт `session/flush`, восстановление после сбоя и `SessionHeader` — на **[persistence.md](persistence.md)**.

## `ToolDefinition`

Единственный относящийся к ядру тип, которым описывается пайплайн: чем является каждый зарегистрированный инструмент — видимый модели `ToolSchema` плюс функция `execute` и необязательные колбэки финального содержимого и UI. Автор инструмента редко конструирует его вручную (DSL `defineTool` строит его с типизированными аргументами), но именно этот контракт держит реестр и через который диспетчеризует цикл.
...
Два паттерна повторяются в каждой подсистеме и документируются однажды, здесь.
...
Два ядровых ID — `CallId` (сопоставляет вызов инструмента с его результатом; dsh-llm) и `SessionId` (разделяемая идентичность живого агента и долговременной сессии; dsh-session). Пакеты возможностей брендируют и свои id, например `JobId` в [jobs.md](jobs.md).
