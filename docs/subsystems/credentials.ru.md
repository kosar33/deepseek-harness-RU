...
`describe(ref)` отвечает поверхностям конфигурации, никогда не раскрывая само значение: разрешается ли ссылка, из какого слоя и завершился бы `set` успехом прямо сейчас. Локальный провайдер помечает ссылку, которую поставляет текущее окружение процесса, значением `writable: false` — запись выглядела бы успешной, хотя разрешение продолжало бы возвращать затеняющее значение, поэтому seam отвергает её, и UI может заранее показать такую ссылку доступной только для чтения.
