...
[English](credentials.md) | [中文](credentials.zh.md) | Русский

Seam учётных данных в [dsh-credentials](../../packages/credentials/credentials) удерживает секреты вне конфигурации: разделы настроек и записи `cordis.yml` несут *ссылки* (имена переменных окружения), значениями владеют провайдеры вроде [dsh-credentials-local](../../packages/credentials/credentials-local), а потребители разрешают ссылку один раз на операцию — LLM-адаптеры делают это один раз на запрос к модели, поэтому сменённые учётные данные достигают самого следующего запроса без какого-либо перезапуска. Единое для всего seam правило связывает всех провайдеров: пустое сохранённое значение отсутствует всюду.

Источник: [`packages/credentials/credentials/src/index.ts`](../../packages/credentials/credentials/src/index.ts)

## Идентичность

Ссылка именует одни учётные данные в виде POSIX-имени переменной окружения. Бренд не позволяет вызывающим смешивать ссылки на учётные данные с прочими строками, передаваемыми между пакетами или процессами; при создании проверяется синтаксис идентификатора оболочки.
...
`resolve(ref)` возвращает значение вместе с определённым провайдером слоем-источником, который его поставил, или `undefined`, пока учётные данные не настроены. Потребители разрешают ссылку заново при каждой операции и никогда не кэшируют её между операциями — именно это чтение на каждую операцию служит механизмом горячего обновления.
...
`describe(ref)` отвечает поверхностям конфигурации, никогда не раскрывая само значение: разрешается ли ссылка, из какого слоя и увенчался ли бы `set` успехом прямо сейчас. Локальный провайдер помечает ссылку, которую поставляет живое окружение процесса, значением `writable: false` — запись выглядела бы успешной, хотя разрешение продолжало бы возвращать затеняющее значение, поэтому seam отвергает её, и UI может заранее показать такую ссылку доступной только для чтения.
...
`credentials/reference-updated (ref)` срабатывает после зафиксированного изменения источника под управлением провайдера — `set`, `unset` или внешней правки, замеченной в хранилище. Изменения окружения самого процесса ненаблюдаемы и никогда не порождают событие. Потребителям это событие не нужно (они разрешают ссылку заново на каждую операцию); оно существует для поверхностей конфигурации, обновляющих значок «настроено».
