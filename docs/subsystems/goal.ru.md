...
Типы, общие для event-sourced сервиса целей и его потребителей политики. Решениями о персистентности и активации владеет [Agent Note домена целей](../../.agents/notes/implemented/feature/2026-07-19-persisted-same-session-goal-domain.md); эта страница фиксирует точные поля и варианты из [`packages/goal/goal/src/types.ts`](../../packages/goal/goal/src/types.ts).
...
Долговечная фаза отвечает на вопрос, что стало с целью. Процессно-локальная активация отдельно отвечает на вопрос, может ли потребитель продолжений начать ещё один Round.
...
Блокировка — единственное долговечное состояние «остановлено из-за проблемы». Принадлежащая политике причина несёт стабильный код в lower-kebab-case для маршрутизации и объяснение в свободной форме для людей и моделей.
...
Каждая мутация — долговечное событие сессии `goal/change`, чья полезная нагрузка — либо полный послемутационный снапшот, либо надгробие очистки (clear). Строгая свёртка и персистентная проекция выводят состояние жизненного цикла только из этих событий; мутации инбокса на состояние цели не влияют.
...
Потребитель продолжений атрибутирует каждый допущенный ход с пользовательским сообщением положительным последовательным номером Round и текущей ревизией; только эти допущенные события `user/message` продвигают `roundsStarted`. Воспроизведение отвергает неположительные номера Round, пропуски, устаревшие ревизии, остановленные фазы и переполнение потолка.
