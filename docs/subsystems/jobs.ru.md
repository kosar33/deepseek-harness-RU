...
`JobStart` объявляет идентичность и функцию запуска. Рантайм завершает предварительную проверку (preflight) до вызова `run()` и фиксирует работу без последующих способных упасть этапов. Ресурсами исполнения владеет продюсер; идентичностью, доступом и состоянием жизненного цикла владеет рантайм.
