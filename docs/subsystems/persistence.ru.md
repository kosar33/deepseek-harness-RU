...
**Seam долговечности** журнала событий. В [session.md](session.ru.md) описана находящаяся в памяти `Session` — append-only журнал событий `SessionEvent`, являющийся источником истины. Эта страница описывает, как этот журнал становится долговечным: абстрактный сервис `SessionPersistence`, его бэкенды, чекпоинт флаша, восстановление после сбоя и заголовок метаданных, который путешествует вместе с журналом. Словарь событий, который несёт журнал, перечислен член за членом в сгенерированном [каталоге событий журнала персистентности](../persistence-catalog.md).
...
`SessionPersistence.locate(meta)` синхронно разрешает независимый артефакт во владении бэкенда, не читая, не создавая и не флашая его. JSONL возвращает абсолютный путь транскрипта внутри своего каталога project/session; SQLite возвращает `undefined`, потому что сессии разделяют одну базу данных. Поэтому возвращённый путь может называть файл, который ещё не существует или ещё не содержит текущий незафлашенный ход; это подсказка расположения, а не авторизация и не гарантия актуальности.
...
Метаданные отдельной сессии путешествуют **отдельно** от журнала событий: версия формата, cwd, родословная и граница засева — это заботы хранилища, а не события беседы, поэтому они остаются вне `SessionEventMap` и никогда не достигают `deriveMessages()`. Заголовок прикрепляется к `Session` через `session.header`.
