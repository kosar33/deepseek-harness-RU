...
[English](session-projection.md) | [中文](session-projection.zh.md) | Русский

Seam session-projection — [capability seam](../capability-seams.ru.md), через который доменные хостовые плагины отдают клиентским носителям полные текущие значения производного от журнала состояния каждой сессии: Service Definition и реестр ([dsh-session-projection](../../packages/session/session-projection), `ctx.sessionProjections`), доменные вкладчики (каждый регистрирует один чистый юнит) и носители (страница хвоста истории в [dsh-host-apiproxy](../../packages/host/apiproxy) и push-фрейм `session/projection`). Это одна необязательная возможность, а не часть стержня agent-loop. Фреймворк управляет — домен вычисляет: реестр один раз подписывается на `session/event` и пропускает каждое зафиксированное событие через свёртку каждого юнита; домены не держат подписок, а клиенты никогда не сворачивают доменные события — они получают готовые значения. Авторитетный дизайн: [RFC session-projection](../../.agents/notes/proposed/architecture/2026-07-27-session-projection-and-command-log.md); контракты управления, кэша и ленты изменений: [README пакета](../../packages/session/session-projection/README.md).

Источник: [`packages/session/session-projection/src/index.ts`](../../packages/session/session-projection/src/index.ts)

## Юнит

`SessionProjectionStateMap` — расширяемая слиянием таблица хостовых состояний свёртки, а `SessionProjectionMap` хранит видимые клиенту полные значения. Домен вносит по одному `ProjectionDefinition` на ключ состояния; блок `wire` делает ключ видимым клиенту, а рендеринг принадлежит slot-системе и никогда — этому слою:
...
Правило события с полным значением несёт нагрузку: событие журнала, несущее состояние, содержит полное состояние после изменения, а не голую дельту — это делает каждую свёртку тривиально дешёвой, а каждое обслуженное значение самодостаточным (для потребителей действует правило «последний побеждает»).

## Снапшот и лента изменений
...
`snapshot(session)` полностью синхронен: носитель читает его в том же тике, что и свой срез страницы, поэтому `asOfSeq` накрывает оба чтения одним номером последовательности. Он возвращает только клиентские представления, и каждое значение проходит `viewSchema` своего юнита перед возвратом. `stateOf(session, key)` читает одно живое хостовое состояние, не вычисляя чужих представлений; вызывающие не должны мутировать заимствованную ссылку. Лента изменений срабатывает один раз на каждый видимый клиенту юнит, чья *ссылка* на состояние изменилась, для каждого зафиксированного события; `apply` должен возвращать ту же ссылку, когда состояние не изменилось.

## Реестр: `ctx.sessionProjections`

`SessionProjectionRegistry` ([сигнатуры](#ctxsessionprojections--sessionprojectionregistry)) владеет управлением: одна подписка на `session/event`, упреждающий `apply` по всем зарегистрированным юнитам и ячейки водяных знаков на каждую сессию и юнит. Ячейки строятся лениво — юнит, зарегистрированный после того, как события уже шли, или сессия старше реестра, сворачивает `init` по находящемуся в памяти журналу при первом касании (событие или чтение). Регистрация — эффект, чей диспоузер едет на fiber вызывающего: ключ выгруженного доменного плагина (вместе со своими кэшированными ячейками) исчезает из последующих прогонов и снапшотов, и клиенты читают это как отсутствие возможности; дубликаты ключей бросают исключение. Доменные плагины регистрируются под `ctx.inject(['sessionProjections'], …)`, чтобы headless-сборки без реестра оставались незатронутыми.
