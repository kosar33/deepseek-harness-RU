...
`SessionLogSnapshot` — полный отсоединённый, проверенный воспроизведением сырой журнал, которым пользуется предпроверка возобновления. `SessionSurfaceSnapshot` — одно наблюдение поверхности точным чтением, а не удерживаемая подписка.
...
`SessionTitleObservation` применяет то же правило атомарного наблюдения к свёртке заголовков, поэтому потребитель авторизации может проверить исходный заголовок, из которого взят заголовок. Пакетные чтения возвращают по одному упорядоченному `SessionTitleObservationResult` на каждый уникальный запрошенный id: операционные сбои остаются локальными для этого id, тогда как отмена отвергает операцию целиком.
...
Массивы фильтров сессий и событий объединяются через AND; значения внутри одного предложения-списка объединяются через OR. Диапазоны включают границы. Предложение `text` события — буквальное сканирование регулярным выражением без учёта регистра Unicode и с гибкостью по пробелам поверх извлечённого семантического текста, независимое от полнотекстовых провайдеров.
...
Объединённый seam `ctx.sessionQuery` имеет две полнотекстовые области видимости. `searchSessions()` группирует корпус по сильнейшему совпавшему событию; `searchEvents()` ищет внутри одной сессии. Запросы привязывают непрозрачный курсор к нормализованному запросу, фильтрам метаданных и лимиту. Скан текста событий намеренно отсутствует среди фильтров метаданных провайдера.
...
В отличие от сгруппированных межсессионных попаданий, поиск внутри сессии обязан открывать и свой наблюдаемый целевой заголовок, даже когда страница не содержит попаданий.
...
Запрос адресует один сырой seq и необязательные счётчики соседей. Результат несёт `SessionHeader`, а не флаги доступности, чтобы известная живая цель могла оставаться независимой от здоровья персистентности.
...
Трассировки событий различают позиционное замещение на поверхности от событий, процитированных как источники. Каждый список seq содержит прямые ссылки, кроме `replacementChain`, который идёт от цели к финальному позиционному замещению через непосредственные замещатели.
...
Закрытое объединение кодов различает невалидный запрос, отсутствующую цель, повреждённую поверхность журнала, отказ опционального бэкенда, отключённый в развёртывании поиск и противоречивые метаданные источника.
