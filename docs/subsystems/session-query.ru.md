...
[English](session-query.md) | [中文](session-query.zh.md) | Русский

Словарь запросов над корпусом логических сессий с приоритетом живых сессий. [Пакет Service Definition](../../packages/session-query/session-query) владеет точными чтениями, приоритетом источников, трассировкой связей, семантическим извлечением и независимыми от провайдеров фильтрами, а [SQLite-провайдер](../../packages/session-query/session-query-sqlite) — конкретным жизненным циклом полнотекстового индекса.

Источник: [`packages/session-query/session-query/src/types.ts`](../../packages/session-query/session-query/src/types.ts)

## Логические записи

`SessionRecord` возвращается списком по всему корпусу. Он открывает доступность источников независимо от клонированного заголовка, выбранного по принципу приоритета живой сессии. `SessionEventRecord` — лёгкая проекция сырого журнала; классификация пользуется теми же переходами `foldSurface()`, что и построение истории модели.
...
`SessionLogSnapshot` — полный отсоединённый, проверенный воспроизведением сырой журнал, которым пользуется предпроверка возобновления. `SessionSurfaceSnapshot` — одно наблюдение поверхности через точное чтение, а не удерживаемая подписка.
...
`SessionTitleObservation` применяет то же правило атомарного наблюдения к свёртке названий, поэтому потребитель авторизации может проверить исходный заголовок, из которого получено название. Пакетные чтения возвращают по одному упорядоченному `SessionTitleObservationResult` на каждый уникальный запрошенный id: операционные сбои остаются локальными для этого id, тогда как отмена отвергает операцию целиком.
...
## Независимые от провайдера фильтры и документы

Массивы фильтров сессий и событий объединяются логическим И; значения внутри одного условия-списка объединяются логическим ИЛИ. Диапазоны включают границы. Условие `text` события — буквальное сканирование регулярным выражением по извлечённому семантическому тексту, без учёта регистра Unicode и с гибкостью по пробельным символам, независимое от полнотекстовых провайдеров.
...
`ctx.sessionQuery.filterSessions(filters)` применяет `SessionResultFilter` к полному логическому корпусу; `ctx.sessionQuery.filterEvents(sessionId, filters)` возвращает совпавшие документы в порядке возрастания seq. Семантический текст дают сообщения, рассуждения, вызовы и результаты инструментов, заблокированные промпты, элементы todo и детали сбоев и статусов; структурные события и чанки потока — нет.

## Страницы полнотекстового поиска

Объединённый seam `ctx.sessionQuery` имеет две полнотекстовые области видимости. `searchSessions()` группирует корпус по сильнейшему совпавшему событию; `searchEvents()` ищет внутри одной сессии. Запросы привязывают непрозрачный курсор к нормализованному запросу, фильтрам метаданных и лимиту. Сканирование текста событий намеренно отсутствует среди фильтров метаданных провайдера.
...
В отличие от сгруппированных межсессионных попаданий, поиск внутри сессии должен также возвращать свой наблюдаемый заголовок целевой сессии, даже когда страница не содержит попаданий.
...
## Происхождение сессии

`SessionLineageTrace` несёт известных родителей в порядке от ближайшего к дальнему и лес рекурсивно вложенных прямых потомков. Дискриминант полноты делает известный корень и отсутствующего родителя взаимоисключающими.
...
## Чтения ограниченных событий

Запрос адресует один сырой seq и необязательные счётчики соседей. Результат несёт `SessionHeader`, а не флаги доступности, чтобы известная живая цель могла оставаться независимой от состояния персистентности.
...
Трассировки событий отличают позиционное замещение на поверхности от событий, указанных как источники. Каждый список seq содержит прямые ссылки, кроме `replacementChain`, который идёт от цели к финальному позиционному замещению через непосредственные замещатели.
...
Закрытое объединение кодов различает невалидный запрос, отсутствующую цель, некорректную поверхность журнала, отказ опционального бэкенда, поиск, отключённый на уровне развёртывания, и противоречивые метаданные источника.
