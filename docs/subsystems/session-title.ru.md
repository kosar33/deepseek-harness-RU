...
## Долговечное состояние заголовка

Для созданных провайдером редакций записывается `SessionTitleProviderId`. `SessionTitleEventData` перечисляет точные seq человеческих сообщений, использованные для заголовка, а `SessionTitleSnapshot` добавляет выбираемые `foldSessionTitle()` долговечные факты конверта события.
...
## Запись вспомогательного запроса

Общий LLM-помощник записывает каждый провалидированный и подлежащий отправке запрос заголовка перед вызовом модели. Полезная нагрузка воспроизводит видимые модели системный промпт и входные сообщения, маршрутизацию, лимит вывода, принадлежность провайдеру и атрибуцию исходных сообщений — даже если генерация позже завершится сбоем.
...
## Вход и выход провайдера

Сервис делает снапшот подходящих сообщений на одну редакцию. Провайдер возвращает только seq из этого запроса; принадлежащее сервису принятие проверяет порядок, нормализует заголовок, соблюдает лимит байтов и дописывает заголовок с seq его исходных сообщений и видом источника.
