...
Seam пользовательских настроек [dsh-settings](../../packages/settings/settings) хранит один принадлежащий пользователю документ из секций по одному на пространство имён и разрешает каждое зарегистрированное пространство имён как значения по умолчанию из схемы, затем композиционный `base` регистранта, затем пользовательскую секцию. Провайдеры вроде [dsh-settings-file](../../packages/settings/settings-file) хранят исходный документ и проталкивают внешние правки; плагины-потребители регистрируют схему и читают или наблюдают разрешённое значение. Конфигурация композиции остаётся в `cordis.yml` — пространство имён несёт только редактируемый пользователем поднабор.

Источник: [`packages/settings/settings/src/index.ts`](../../packages/settings/settings/src/index.ts)

## Идентификация

Пространство имён именует одну принадлежащую плагину секцию пользовательского документа. Бренд не позволяет вызывающим смешивать пространства имён настроек с другими id, передаваемыми между пакетами или процессами; при конструировании проверяется синтаксис строчного kebab-case.
...
Регистрация связывает схему schemastery с пространством имён на fiber'е вызывающего плагина — освобождение этого fiber'а удаляет пространство имён вместе с его наблюдателями. Опции несут слой композиции, тайминг эффектов владельца и необязательную проверку для того, что схема не может выразить.
...
`validate` выполняется после того, как значение пропущено схемой, поэтому видит значения по умолчанию и композиционный base в точности так, как их увидит владелец. `dsh-llm-pi-ai` использует её, чтобы отвергнуть профиль провайдера, который она не смогла бы обслужить, ещё в той записи, которая его породила, вместо сохранения профиля, отключившего бы каждый маршрут её пространства имён.
...
`describe()` сериализует каждое зарегистрированное пространство имён для конфигурационных поверхностей: конверт schemastery `toJSON()` управляет формами, отрисованными по схеме, разрешённое значение их заполняет, а отсоединённые слои `base`/`user` позволяют форме помечать переопределённые пользователем поля по самому факту присутствия. `describe({ redactSecrets: true })` — обязательный на каждой поверхности протокола — вырезает поля `role('secret')` из всех трёх слоёв и перечисляет их слоты `{path, set}`, чтобы страница могла отрисовать write-only-поля ввода, ни разу не получив секрет.
...
Вызывающий, владеющий только очищенным от секретов дескриптором, не может безопасно пересобрать секцию, поэтому удаления передаются вместо этого как path-операции. Каждый дескриптор также несёт `revision` исходной секции; запись может отправить его обратно как `expectedRevision`, и запись, которая уже не совпадает, отвергается, а не применяется поверх зафиксировавшей первой.
```ts type-equiv
/**
...
Каждое зафиксированное изменение — внутрипроцессная запись или внешне наблюдаемая правка от провайдера — порождает `settings/updated (ns, next, prev, source)` после того, как новое значение становится авторитетным, и никогда не порождается, если разрешённое значение deep-equal. Тег источника разделяет два пути входа.
