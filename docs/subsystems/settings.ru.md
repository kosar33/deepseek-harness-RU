...
[English](settings.md) | [中文](settings.zh.md) | Русский

Seam пользовательских настроек [dsh-settings](../../packages/settings/settings) хранит один принадлежащий пользователю документ из секций — по одной на пространство имён — и разрешает каждое зарегистрированное пространство имён как значения по умолчанию из схемы, затем композиционный `base` регистранта, затем пользовательскую секцию. Провайдеры вроде [dsh-settings-file](../../packages/settings/settings-file) хранят исходный документ и публикуют внешние правки; плагины-потребители регистрируют схему и читают или наблюдают разрешённое значение. Конфигурация композиции остаётся в `cordis.yml` — пространство имён несёт только редактируемый пользователем поднабор.

Источник: [`packages/settings/settings/src/index.ts`](../../packages/settings/settings/src/index.ts)

## Идентификация

Пространство имён именует одну принадлежащую плагину секцию пользовательского документа. Бренд не позволяет вызывающим смешивать пространства имён настроек с другими id, передаваемыми между пакетами или процессами; при конструировании проверяется синтаксис kebab-case в нижнем регистре.
...
Регистрация связывает схему schemastery с пространством имён на fiber'е вызывающего плагина — освобождение этого fiber'а удаляет пространство имён вместе с его наблюдателями. Опции несут слой композиции, момент вступления изменений в силу у владельца и необязательную проверку того, что схема выразить не может.
...
`validate` выполняется после того, как значение пропущено схемой, поэтому видит значения по умолчанию и композиционный `base` в точности так, как их увидит владелец. `dsh-llm-pi-ai` использует её, чтобы отклонить профиль провайдера, который пакет не смог бы обслужить, уже на этапе записи, которая его создаёт, вместо сохранения профиля, отключающего каждый маршрут этого пространства имён.

`applies` — подсказка для UI, а не механизм: владелец `restart` просто никогда не наблюдает, поэтому его значение читается один раз при конструировании, а конфигурационные поверхности могут помечать ещё не применённое изменение.
...
Скоуп — дескриптор для владельца. `update` накладывает разреженный патч только поверх пользовательской секции (никогда — в `base`); `replace` задаёт секцию целиком, это путь удаления/сброса — ключи, отсутствующие в замене, заново наследуют `base` и значения по умолчанию из схемы. Записи в одно пространство имён сериализуются в порядке вызовов, а разрешённые значения — глубокозамороженные снапшоты.
...
`describe()` сериализует каждое зарегистрированное пространство имён для конфигурационных поверхностей: конверт schemastery `toJSON()` управляет отрисовкой форм по схеме, разрешённое значение их заполняет, а отсоединённые слои `base`/`user` позволяют форме помечать переопределённые пользователем поля по их наличию. `describe({ redactSecrets: true })` — обязателен на каждой поверхности протокола — вырезает поля `role('secret')` из всех трёх слоёв и перечисляет их слоты `{path, set}`, чтобы страница могла отрисовать поля ввода «только для записи», ни разу не получив секрет.
...
Вызывающий, располагающий только очищенным от секретов дескриптором, не может безопасно пересобрать секцию, поэтому удаления передаются вместо этого как path-операции. Каждый дескриптор также несёт `revision` исходной секции; операция записи может вернуть его как `expectedRevision`, и если ревизия уже не совпадает, такая запись отвергается, а не применяется поверх записи, зафиксировавшейся первой.
```ts type-equiv
/**
...
Каждое зафиксированное изменение — внутрипроцессная запись или внешне наблюдаемая правка от провайдера — порождает `settings/updated (ns, next, prev, source)` после того, как новое значение становится авторитетным, и никогда не порождается, если разрешённое значение глубоко совпадает с прежним. Тег источника разделяет два пути поступления изменений.
