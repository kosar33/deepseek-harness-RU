...
Реестр владеет только своим ограничением кэша обнаружения. Локальный провайдер владеет корнями файловой системы (`dshHome`, `agentsHome`, `customSkillDirs` и опциональными `bundledSkillDir`/`DSH_BUNDLED_SKILL_DIR`) плюс параметрами наблюдателя: включением, polling, стабилизацией, symlink и проектной ёмкостью. Потребитель владеет своим ограничением длины описания каталога. Точные значения по умолчанию и валидация — в сгенерированном [каталоге конфигурации](../config-catalog.md).
