...
`ToolProviderResult.schemas` — видимое модели множество схем текущей сборки. `knownNames` — исходная вселенная имён провайдера до наложения ограничений; она позволяет отличить опечатку в настроенном имени от известного инструмента, намеренно скрытого в этой области действия.
...
`PromptSection` — контракт регистрации только для чтения, действующий внутри одного процесса. Его текст статичен либо вычисляется из текущего контекста сборки. Один действующий раздел `complete` после кооперативной сборки становится единственной секцией промпта.
