...
Реестр сохраняет протокольную декларацию поиска после выгрузки его резолвера. Поэтому SRC-discovery продолжает классифицировать параметр как поиск и завершается статусом unavailable вместо того, чтобы принять значение из протокола как обычный бизнес-объект.
...
Сгенерированные потребительские декларации вливают прямые пространства имён в словарь, наследуемый `TypertClientRemote`.
...
Connection декодирует конверт носителя до вызова `ctx.typertGateway`. Запрос несёт точные именованные поля протокола и сигнал отмены носителя по отдельности; инфраструктурные и граничные отказы используют внутрипроцессную таксономию ошибок шлюза, обычные исключения RPC-адаптер сворачивает в транспортный код ошибки `internal`, а уже существующие RPC-ошибки, проводимые политикой поиска через `TypertLookupFailure`, возвращаются без изменений.
