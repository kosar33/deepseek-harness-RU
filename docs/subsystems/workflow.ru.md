...
Что запрашивает вызывающий при старте запуска. Обычный инструмент воркфлоу собирает его из вызова модели `{ script, meta, args }` плюс вызывающего агента; специализированные потребители могут также выбрать один общий для движка `subagentProvider` и уменьшить `maxTotalAgents` для этого запуска, но скрипт не может ни наблюдать, ни заменить ни одну из этих политик. `meta` и `args` — обычные JSON-ДАННЫЕ (движок проверяет `meta` по его схеме и отвергает запрос с явной ошибкой ДО того, как что-либо исполнится, — ради их получения текст скрипта никогда не вычисляется). `parent` **ОБЯЗАТЕЛЕН** — каждый потомок, запущенный скриптом, атрибутируется ему, а cwd, lineage и глубина проходят через [seam субагентов](subagent.ru.md).
...
Итог одного запуска, разрешаемый через `WorkflowRun.result`. `value` — материализованное возвращаемое значение скрипта — обычные JSON-данные мира хоста (`null`, когда скрипт ничего не вернул) — осмысленное только для `completed`. `stopReason` — ЗАМКНУТОЕ объединение (владелец — движок; потребители могут перебрать все его варианты): `completed` | `cancelled` | `error`. Причина, отличная от `completed`, несёт сбой в `error`, а потребитель отображает её в результат инструмента `isError`, а не выдаёт частичный вывод за успех.
