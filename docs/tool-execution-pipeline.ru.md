...
Проверки чтения-до-правки файловой системы остаются ниже `tool-fs`, на событиях `fs/*`. Обобщённые каскады pre/post размещают хуки и политику одобрения; `ctx.approval` разбирает запросы одобрения до монотонных guard'ов, а политика владельца, которую нельзя переупорядочивать, остаётся зарегистрированным guard'ом. Заботы around-диспетчеризации вроде таймаутов оборачивают `tools/execute`. Реестр делает снапшот кандидата-результата без потерь и нормализует сбой снапшота до того, как снапшотированный колбэк `finalizeContent` видимого определения обеспечит свой синхронный content-only инвариант. Затем `tools/result` наблюдает неизменяемый исход в lossless-JSON. Это позволяет хукам простираться через семейства инструментов, не привязывая инструменты к одному сервису политики. Code Mode проводит через пайплайн и зарезервированный транспорт `run_code`, и его сериализованные подвызовы; подвызовы несут токен родителя, логируют `tool/code-dispatch`, возвращают отказы как binding rejections и опускают `additionalContexts`, чтобы сохранить смежность вызова и результата.

Режим сопровождения: курируемый Mermaid-граф; точные схемы инструментов и сигнатуры событий живут в сгенерированных каталогах.
