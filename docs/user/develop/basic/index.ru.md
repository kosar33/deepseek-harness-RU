...
Этот туториал создаёт минимальный плагин Harness и загружает его в веб-интерфейс. Начните с рабочей копии репозитория, в которой уже выполнены шаги [запуска из исходников](../../../../README.ru.md#запуск-из-исходников).
...
Откройте `http://127.0.0.1:3080`. Во время старта терминал выводит `[hello-plugin] plugin loaded!`.

## Автоматическая очистка

Всё, что зарегистрировано через `ctx` — слушатели событий, инструменты или таймеры, — освобождается при выгрузке плагина. Вам не нужно вручную вызывать removeListener или clearInterval.
...
- [Туториал по Cordis](../../../cordis-tutorial/index.ru.md) — фреймворк плагинов под капотом, строится с нуля без API-ключа
