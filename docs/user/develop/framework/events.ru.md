...
События — основной механизм взаимодействия между плагинами Cordis. Harness широко использует их для слабосвязанных точек расширения.
...
Слушатели выполняются по порядку; первый результат, отличный от `null`, `false` или `undefined`, становится итоговым:
...
Каждый слушатель может обернуть результат ниже по цепочке, образуя цепочку обработки. Слушатель **ДОЛЖЕН вызвать `next()` для передачи дальше**; пропуск вызова замыкает конвейер накоротко:
...
::: warning
Слушатель waterfall **ДОЛЖЕН вызвать `next()`**. Пропуск вызова замыкает конвейер накоротко намеренно — так реализуются перехват и поведение шлюза.
:::
...
События Cordis в Harness используют имена `namespace/action`, включая `agent/step`, `agent/request`, `agent/request-error`, `tools/result` и `session/event`. Сгенерированные области `cordis-surface` на [страницах подсистем](../../../subsystems/core.ru.md) фиксируют полные сигнатуры и режимы.

`turn/*`, `step/*`, `tool/call`, `tool/result` и `compaction/*` — персистентные типы событий сессии, а не одноимённые события Cordis. Чтобы отслеживать их, подпишитесь на `session/event` и проверьте `event.type`.
