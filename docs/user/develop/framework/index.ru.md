...
Если требуемый сервис исчезает, например при подмене провайдера, плагин выгружается автоматически (ACTIVE → DISPOSED) и загружается снова, когда сервис возвращается.
...
Фреймворк отслеживает и освобождает все эти операции:
...
Гарантии `dispose`:
1. Все регистрации, которыми владеет плагин, удалены.
2. Дочерние плагины выгружены рекурсивно.
3. Возвращённый промис разрешается после завершения всей асинхронной очистки.

## Горячая подмена (HMR)

С загруженным из `cordis.yml` пакетом `@deepseek-ai/cordis-plugin-hmr` правка файла исходника плагина запускает:

1. Выгрузку старого плагина и очистку его регистраций.
2. Загрузку нового кода.
3. Выполнение нового `apply`.

Поскольку регистрации плагина очищают себя сами, горячая подмена не сохраняет регистраций старого экземпляра.
...
- [Система событий](./events.ru.md) — общайтесь между плагинами
- [Туториал по Cordis](../../../cordis-tutorial/index.md) — тот же жизненный цикл, сервисы и события, построенные шаг за шагом на рантайме Cordis
