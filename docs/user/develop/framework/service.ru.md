...
Любой плагин может предоставить сервис для других плагинов.
...
Когда выполняется `apply`, каждый сервис, объявленный в `inject`, готов. Если сервис не готов, плагин ожидает, а не запускается.
...
Если требуемый сервис исчезает во время работы приложения, например потому что его провайдер выгружается:
...
`plugin-a` и `plugin-b` видят экземпляр Bash своей собственной группы, без межгрупповых эффектов.

## Встроенные сервисы Harness

Имена сервисов, их публичные методы и расположение исходников генерируются на [страницы подсистем](../../../subsystems/core.ru.md). При разработке плагина пользуйтесь этими сгенерированными областями и интерфейсом TypeScript сервиса; не ведите второй статический список.

## Дальнейшие шаги

- [Система событий](./events.ru.md) — обмен событиями между плагинами без жёсткой связанности
