...
Переопределите `resolveModel(provider, model, signal?)`, чтобы вернуть точную идентичность провайдера/модели плюс опциональные метаданные `context` и `reasoning` одним запросом. Метаданные рассуждений содержат упорядоченные непрозрачные id и отображаемые имена плюс опциональное настроенное значение по умолчанию; сохраняйте канонический список выбираемых значений адаптера, включая `off`, если вышестоящий API возможностей его возвращает, вместо того чтобы переносить эти значения в перечисление ядра. Учитывайте опциональный сигнал при асинхронном запросе, чтобы отмена и освобождение ресурсов достигали quiescence (полного завершения всех жизненных циклов). Сервис валидирует итоговую структуру и отклоняет неподдерживаемые явно заданные усилия рассуждений до `stream()`; пропуск `reasoning` означает, что у этой модели нет выбираемой возможности усилия рассуждений.
