<!-- Сгенерировано скриптом scripts/gen-doc-graphs.ts — не редактируйте вручную.
     Для регенерации запустите `pnpm run gen-doc-graphs`. -->

# Композиция снапшотов Headless Agent

Композиция снапшотов headless сочетает реальный адаптер DeepSeek и возможности написания кода с одним явно сконфигурированным персистентным агентом верхнего уровня; её JSONL-драйвер предназначен только для тестов.
...
| Идентификатор плагина | Пакет / модуль |
| --- | --- |
...
Исходная конфигурация: [`examples/headless-agent/cordis.yml`](cordis.yml).

Режим сопровождения: гибридный: список листовых плагинов разбирается из его `cordis.yml`, а раскрытие пакетов приложения поддерживается вручную по исходному коду пакетов.
