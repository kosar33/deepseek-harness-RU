...
Лаунчер [Landlock](https://landlock.io/) в духе «сначала самоограничься, затем exec» для удержания дочерних процессов в ограничениях на Linux, распространяемый как предсобранные помашинные npm-пакеты плюс тонкий входной JS-пакет, который находит бинарник и владеет его CLI-контрактом. Создан для agent harness и других хостов, которым нужно исполнять недоверенные команды под белым списком файловой системы, не ограничивая самих себя.
...
- `LAUNCHER_BIN` и `LAUNCHER_FAILURE_EXIT` (125): контрактные константы. Ребёнок, удачно запущенный через exec, тоже может вернуть 125, поэтому для атрибуции отказа лаунчера потребителю нужны фатальная диагностика вместе со статусом.
...
Бинарники игнорируются git и собираются нативно по архитектурам — локально для собственной машины либо пер-архитектурными раннерами CI как официальными сборщиками. Порядок выпуска: [docs/release.md](docs/release.md).
