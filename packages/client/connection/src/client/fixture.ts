...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    llm: {
      ...
      // The fixture's one parked key clears into the usable-only snapshot.
      keyRotationResetParks: request => ok(request, {
        configured: true,
        routes: [{
          provider: 'openrouter',
          activeLabel: 'OPENROUTER_KEYROTATION_2',
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'llm.keyRotationResetParks': return this.api.llm.keyRotationResetParks(request)
    }
  }
