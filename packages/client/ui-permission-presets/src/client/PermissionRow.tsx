...
import { FULL_ACCESS_PRESET, presetModeKey } from './presentation.ts'
...
export function PermissionRow({ load, select, usePermission, t }: PermissionRowProps) {
  ...
  const localizedLabel = (option: { id: string; label: string }): string => {
    const modeKey = presetModeKey(option.id)
    return modeKey !== undefined ? t(modeKey) : option.label
  }
  const label = (selected !== undefined ? localizedLabel(selected) : undefined)
    ...
    <>
      <div className={css.row}>
        ...
        <Menu
          ...
          items={state.options.map(option => ({ id: option.id, label: localizedLabel(option) }))}
