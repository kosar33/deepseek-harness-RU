...
export interface ReadBlockLabels {
  /** Banner note for a windowed read: how many of the file's lines were returned. */
  windowed: (shown: number, total: number) => string
  ...
  collapse: string
  /** Toggle text while capped, given the hidden line count. */
  expand: (hidden: number) => string
}

const DEFAULT_LABELS: ReadBlockLabels = {
  windowed: (shown, total) => `显示 ${shown} / ${total} 行`,
  copy: '复制',
  copied: '复制成功',
...
export function ReadBlock({
  ...
  const L: ReadBlockLabels = { ...DEFAULT_LABELS, ...labels }
    ...
    <div className={clsx(css.block, className)} data-read="">
      <div className={css.banner}>
        ...
        <div className={css.action}>
          {windowed && (
            <span className={css.count}>{L.windowed(lines.length, totalLines)}</span>
          )}
