...
行上方是实时池健康的逐密钥标签：粘滞位置的密钥高亮为**使用中**，其余为**可用**或**已停用**；已停用者根据快照的 `resetAt` 本地计算倒计时（「限额将在 N 小时 M 分后重置」），随墙钟推进。行即顺序——密钥自上而下依次尝试；箭头调整顺序而不会移动行已绑定的存储引用。保存回路沿用「模型」页的信任模型：输入的值经 `credentials.set` 只写不读，存放在派生的 `<ROUTE>_KEYROTATION_<n>` 引用下（取现有最大索引加一，因此调序绝不会让引用改指另一把机密）；一条整数组 `keys` 操作只记录这些引用名——设置文档永不携带机密。删除全部行会 unset 该路由的用户层条目；被移除的引用会 unset 其凭据。全新的空白行会在任何 wire 调用之前以指名字段的提示阻止保存；两行包含相同值同样阻止——一把机密不应存于两个引用之下。保存按钮旁的「重置停用时限」胶囊经 `llm.keyRotationResetParks` 清除该路由的全部生效停用并重载快照；路由上没有停用时保持禁用。不可服务的写入在写入处被拒（插件在 `settings.mutate` 校验），因此槽位展示 Host 的报错而不是静默停用路由。
