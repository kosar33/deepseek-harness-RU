...
export const ru: { [Key in keyof typeof en]: string } = {
  nav: 'Модели',
  title: 'Модели',
  intro: 'Укажите свои API-ключи, чтобы пользоваться моделями этих провайдеров.',
  edit: 'Изменить',
  editProvider: 'Изменить «{provider}»',
  remove: 'Удалить',
  removeProvider: 'Удалить «{provider}»',
  deleteTitle: 'Удалить «{provider}»?',
  deleteDescription: 'Удаление «{provider}» стирает его конфигурацию. Используемые им учётные данные управляются отдельно и будут сохранены.',
  deleteDescriptionWithCredential: 'Удаление «{provider}» стирает его конфигурацию и сохранённый API-ключ.',
  ...
  apply: 'Применить',
  applying: 'Применение…',
  savedProvider: 'Настройки «{provider}» сохранены.',
  ...
  conflict: 'Пока карточка была открыта, эти настройки изменились в другом месте. Закройте её и откройте заново, чтобы редактировать актуальные значения.',
  retry: 'Повторить',
  keyInput: 'API-ключ',
  keyPlaceholder: 'Введите свой API-ключ',
  keyPlaceholderNative: 'Введите API-ключ или оставьте поле пустым, чтобы использовать аутентификацию через окружение',
  keyStored: 'Уже задан — введите новое значение, чтобы заменить',
  keyEnvLocked: 'Задаётся средой запуска (только для чтения)',
  ...
  modelsEmpty: 'Ни одна модель не будет показана в селекторе. Незарегистрированные ID всё равно можно отправлять напрямую.',
  ...
  modelCapacityInvalid: 'Лимит должен быть числом, при необходимости с суффиксом K или M.',
  ...
  fetching: 'Запрашиваем провайдера…',
  fetchNeedsBaseUrl: 'Сначала укажите базовый URL, затем выполните запрос.',
  fetchEmpty: 'Провайдер не вернул ни одной модели. Добавьте их вручную.',
  fetchTitle: 'Выберите модели для добавления',
  fetchDescription: 'Это модели, доступные у данного провайдера. Отметьте те, которые нужно добавить.',
  ...
  customRouteHint: 'Идентификатор из строчных букв, начинающийся с буквы: он уникально именует провайдера в запросах и служит именем его учётных данных.',
  ...
  customNeedsModels: 'Пользовательскому провайдеру нужна хотя бы одна модель.',
  ...
  onboardingDescription: 'Настройте официального провайдера DeepSeek и начинайте работу.',
