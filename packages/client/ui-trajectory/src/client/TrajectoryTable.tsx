...
/** Dictionary key per record kind; the label renders through the bound translate. */
...
/** Localized badge for one record kind. */
function kindLabel(kind: TrajectoryCellKind, t: Translate): string {
...
const SYSTEM_PROMPT_TABS: readonly DetailTabItem[] = [
  { id: 'system-prompt', key: 'tab.systemPrompt' },
...
const REQUEST_TABS: readonly DetailTabItem[] = [
  { id: 'overview', key: 'tab.summary' },
  { id: 'options', key: 'tab.options' },
  { id: 'usage', key: 'tab.usage' },
  { id: 'timing', key: 'tab.timing' },
]
...
function formatStartedAt(timestamp: number | null, t: Translate): string {
  if (timestamp === null || !Number.isFinite(timestamp)) return t('value.notAvailable')
...
function StartedAtValue({ timestamp, t }: { timestamp: number | null; t: Translate }) {
  const [showUnix, setShowUnix] = useState(false)
  if (timestamp === null || !Number.isFinite(timestamp)) {
    return <dd>{t('value.notAvailable')}</dd>
  }
  return (
    <dd>
      <button
        ...
        title={showUnix ? t('timing.showLocalTime') : t('timing.showUnix')}
        onClick={(event) => {
...
function totalTime(metrics: AssistantMetricDetail, t: Translate): string {
...
function ttft(metrics: AssistantMetricDetail, t: Translate): string {
  if (!metrics.timingRecorded) return t('timing.notRecorded')
  if (metrics.stepStartTime === null) return t('timing.stepStartUnavailable')
...
function generationTime(metrics: AssistantMetricDetail, t: Translate): string {
...
function throughput(metrics: AssistantMetricDetail, t: Translate): string {
...
function AssistantTimingPanel({ metrics, t }: { metrics: AssistantMetricDetail; t: Translate }) {
  return (
    <dl className={css.overview}>
      <div><dt>{t('field.started')}</dt><StartedAtValue timestamp={metrics.stepStartTime} t={t} /></div>
      <div><dt>{t('field.totalDuration')}</dt><dd>{totalTime(metrics, t)}</dd></div>
      <div><dt>{t('field.ttft')}</dt><dd>{ttft(metrics, t)}</dd></div>
      <div><dt>{t('field.generation')}</dt><dd>{generationTime(metrics, t)}</dd></div>
      <div><dt>{t('field.throughput')}</dt><dd>{throughput(metrics, t)}</dd></div>
    </dl>
  )
}
...
function sectionLabel(turn: number | null, t: Translate): string {
  return turn === null ? t('section.betweenTurns') : t('section.turn', { turn })
}
...
function summarizeTurn(records: readonly TableRecord[], t: Translate): string {
  const steps = new Set(
    records
  ...
  return [
    t('summary.steps', { count: steps }),
    t('summary.toolCalls', { count: toolCalls }),
  ].join(' · ')
}
...
function summarizeAssistantTools(records: readonly TableRecord[], t: Translate): string {
  ...
  const summary = t('summary.toolCalls', { count })
...
function statusLabel(state: RecordState, t: Translate): string {
  if (state === 'error') return t('status.failed')
  if (state === 'running') return t('status.pending')
  return t('status.completed')
}

function TokenRows({ cell, t }: { cell: TrajectoryCellProps; t: Translate }) {
...
function UsageRows({ usage, t }: { usage: TrajectoryUsage | undefined; t: Translate }) {
  ...
  return (
    <dl className={css.overview}>
      {totalInput !== undefined && (
        <div><dt>{t('field.input')}</dt><dd>{totalInput} tok</dd></div>
      )}
      {usage.cacheRead !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>{t('field.cached')}</dt>
          <dd>{usage.cacheRead} tok</dd>
        </div>
      )}
      {usage.cacheWrite !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>{t('field.cacheCreated')}</dt>
      ...
      {usage.output !== undefined && (
        <div><dt>{t('field.output')}</dt><dd>{usage.output} tok</dd></div>
      )}
...
function RequestOptions({
  ...
  return (
    <JsonTree
      data={options}
      label={t('json.requestOptions')}
      className={preview ? css.jsonPreview : css.jsonPayload}
    />
  )
}

function messageSourceLabel(source: unknown, t: Translate): string {
...
function MessageSource({ record, t }: { record: TableRecord; t: Translate }) {
...
function detailTabs(record: TableRecord): readonly DetailTabItem[] {
  ...
  if (record.cell.kind === 'compacted') {
    return [
      { id: 'overview', key: 'tab.summary' },
      { id: 'raw', key: 'tab.rawOutput' },
    ]
  }
  if (isMarkdownRecord(record)) {
    return [
      { id: 'overview', key: 'tab.summary' },
      { id: 'rendered', key: 'tab.preview' },
      { id: 'raw', key: 'tab.raw' },
      ...(record.cell.messageSource === undefined
        ? []
        : [{ id: 'source', key: 'tab.source' } as const]),
    ]
  }
  return [
    { id: 'overview', key: 'tab.summary' },
    ...(record.cell.inputDetail ? [{ id: 'input', key: 'tab.payload' } as const] : []),
    ...(record.cell.outputDetail ? [{ id: 'output', key: 'tab.result' } as const] : []),
    { id: 'schema', key: 'tab.schema' },
    { id: 'timing', key: 'tab.timing' },
  ]
}
...
function RecordListText({
  ...
  toolCallText,
  t,
}: Pick<RecordPresentationValue, 'displayText' | 'toolCallOnly' | 'toolCallText'> & { t: Translate }) {
  if (toolCallOnly) {
    return <span className={css.toolCallOnly}>{t('row.toolCallOnlyHint')}</span>
  }
          ...
          {block.callId !== undefined
            ? (
              <button
                ...
                aria-label={t('a11y.openBlockCall', { index: index + 1 })}
                title={t('a11y.openToolCallSummary')}
                onClick={() => {
                  if (block.callId !== undefined) onOpenCall(block.callId)
                }}
              >
                <span className={css.sourceBlockLabel}>
                  {t('detail.blockLabel', { index: index + 1, type: block.type })}
                </span>
...
function ToolCatalog({ tools, t }: { tools: ConversationPromptSnapshot['tools']; t: Translate }) {
  if (tools.length === 0) return <p className={css.noPayload}>{t('empty.noTools')}</p>
  return (
        ...
        <details className={css.toolCatalogItem} key={`${tool.name}:${index}`}>
          ...
          <div className={css.toolCatalogDefinition}>
            ...
            <JsonTree
              ...
              label={t('json.toolParameters', { name: tool.name })}
...
function MarkdownRecordContent({
  ...
  if (!source && !hasImages && !hasToolCalls) {
    const emptyLabel = isToolCallOnly(record.cell)
      ? t('detail.toolCallOnly')
      : record.cell.text || t('empty.noContent')
...
function RecordTiming({ record, t }: { record: TableRecord; t: Translate }) {
  return record.cell.kind === 'message' && record.cell.assistantMetrics !== undefined
    ...
    : (
      <dl className={css.overview}>
        <div><dt>{t('field.started')}</dt><StartedAtValue timestamp={record.cell.startedAt ?? null} t={t} /></div>
        <div><dt>{t('field.duration')}</dt><dd>{formatElapsedSeconds(record.cell.timeSeconds)}</dd></div>
        <div>
          <dt>{t('timing.source')}</dt>
          <dd>{record.cell.timeSeconds === null ? t('value.notAvailable') : t('timing.sessionTimestamps')}</dd>
        </div>
      </dl>
    )
}
...
function RequestTiming({
  ...
  if (request?.startedAt !== undefined) {
    ...
    return (
      <dl className={css.overview}>
        <div><dt>{t('field.started')}</dt><StartedAtValue timestamp={request.startedAt} t={t} /></div>
        <div><dt>{t('field.duration')}</dt><dd>{formatElapsedSeconds(duration)}</dd></div>
        <div>
          <dt>{t('timing.source')}</dt>
  ...
  return (
    <dl className={css.overview}>
      ...
      <div><dt>{t('field.duration')}</dt><dd>{formatElapsedSeconds(null)}</dd></div>
    </dl>
  )
}
...
function RecordPayload({
  ...
  if (json !== undefined) {
    return (
      <JsonTree
        data={json}
        label={direction === 'input' ? t('json.payload') : t('json.result')}
        className={payloadClassName}
      />
    )
  }
...
function RecordSchema({
  ...
  if (!record.cell.schemaDetail) {
    return <p className={css.noPayload}>{t('schema.unavailable')}</p>
  }
    ...
    return (
      <div className={preview ? `${css.schema} ${css.schemaPreview}` : css.schema}>
        ...
        <section className={css.schemaParameters}>
          <h4 className={css.schemaParametersTitle}>{t('schema.parameters')}</h4>
          <JsonTree
            data={schema.parameters}
            label={t('json.toolParameters', { name: schema.name })}
    ...
    <div ref={rootRef} className={css.split} style={splitStyle}>
              ...
              >
                ...
                ?? (record.cell.isError === true ? 'error' : undefined)
                  ...
                  const requestLabel = request === undefined
                    ? undefined
                    : `${t('label.requestNumber', { number: request })}${
                      requestInfo?.purpose === 'compaction'
                        ? ` · ${t('purpose.compaction')}`
                        : ''
                    }`
                  const rowAriaLabel = isCollapsedSummary
                    ? t(record.collapsedSummaryKind === 'turn'
                      ? 'a11y.collapsedTurnSummary'
                      : 'a11y.collapsedAssistantSummary', { summary: record.collapsedSummary ?? '' })
                    : isRequestOnly
                      ? t('a11y.requestOnlyRow', { number: request ?? '' })
                      : [
                        request === undefined
                          ? ''
                          : t('label.requestNumber', { number: request }),
                        kindLabel(record.cell.kind, t),
                        listDisplayText || t('empty.noContent'),
      ...
      {(selectedRequest !== null
        ...
        >
                ...
                ? (
                  <>
                    ...
                    <span className={css.requestDetailsName}>
                      {t('label.requestNumber', { number: selectedRequestNumber ?? '—' })}
                    </span>
                    <span className={css.detailsLocation}>
                      {selectedRequestInfo?.purpose === 'compaction'
                        ? `${t('purpose.compaction')} · ${sectionLabel(selectedRequest.turn, t)}`
          ...
          <div className={css.detailTabs} role="tablist" aria-label={t('a11y.eventDetails')}>
          ...
          >
                    ...
                    <div>
                      ...
                      <dd>
                        {selectedRequestInfo.maxRetries === undefined
                          ? t('retry.scheduled', { retry: selectedRequestInfo.retry })
                          : t('retry.scheduledOf', {
            ...
            {promptSelected && activeTab === 'system-prompt' && (
              selectedPrompt.system === ''
                ? <p className={css.noPayload}>{t('empty.noSystemPrompt')}</p>
                : (
                    ...
                    <div>
                      ...
                      <dd className={css.overviewParentLinks}>
                        {selectedAssistantRequestTarget !== undefined && (
                          ...
                          >
                            <span>
                              {t('label.requestNumber', { number: selectedAssistantRequest ?? '—' })}
                            </span>
                            <IconChevronRightOutline14
                              className={css.overviewHierarchyJumpIconTight}
                              size={11}
                        ...
                        {selectedParentMessage !== undefined && (
                          ...
                          >
                            <span>{t('badge.assistantMessage')}</span>
                            <IconChevronRightOutline14
                              className={css.overviewHierarchyJumpIconTight}
                              size={11}
                            />
