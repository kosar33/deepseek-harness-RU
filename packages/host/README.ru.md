...
Хостовая сторона веб-GUI dsh: шлюз API, общий для всех клиентских форм, и простой HTTP-сервер, который этот шлюз несёт. Браузерная сторона живёт в [`client/`](../client/README.ru.md); собранное приложение — это [`apps/cli`](../../apps/cli/README.md), загружающее [бандл `dsh-base`](../bundle/base/cordis.patch.yml) и обслуживающее [`apps/web`](../../apps/web/). Все эти пакеты — **продуктовые**.
...
| [`frontend-static/`](frontend-static/README.ru.md) | Сервер SPA-дистрибутива на запасном месте веб-сервера | потребляет `ctx.webServer` |
...
| [`plugin-inventory/`](plugin-inventory/README.ru.md) | Проекция текущих записей Loader только для чтения | удалённый `pluginInventory/list` |
