...
- **Reset precision depends on what survives flattening** — through the pi-ai path, wire errors arrive flattened to message text: headers are gone, so `providerRetryAfterMs` is absent and precision rests on markers providers embed in the payload itself (`reset_at`, `retry-after`, "try again in Ns"); bodies without any marker park until the coming UTC midnight, and the upstream shared-pool pass-through reads that same flattened body by its stable marker substring.
