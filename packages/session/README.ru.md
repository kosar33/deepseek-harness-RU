...
Долговечное семейство вокруг живого внутрипамятного сервиса `core/session`: seam персистентности со своими бэкендами хранения и политикой чекпоинтов, seam проекции, поставляющий целые производные от журнала значения, основанные на журнале заголовки и исходящую телеметрию сессий. Все пакеты — **продуктовые**. `session-query/` остаётся соседней группой: поверхность чтения/инструментов потребляется независимо от внутренностей персистентности.
...
| Пакет | Роль | Ключ ctx |
|---|---|---|
| [`session-title/`](session-title/README.ru.md) | Владеет состоянием заголовка, поведением отката, регистрацией провайдеров и обновлением | `ctx.sessionTitle` |
...
Развёртывания могут зарегистрировать один провайдер на базе модели; при его отсутствии сервис удерживает детерминированный запасной вариант.
...
| Пакет | Роль |
|---|---|
| [`session-telemetry/`](session-telemetry/README.ru.md) | Определяет захват, очистку, проекцию и живую или по требованию доставку бэкенду. |
