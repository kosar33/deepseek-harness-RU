...
Семейство возможностей охватывает канонический seam исполнителя, его реализации, разделяемую среду оболочки и обращённые к модели инструменты. Все они — **продуктовые** пакеты.
...
| [`pwsh-local/`](pwsh-local/README.ru.md) | Исполняет команды PowerShell со специфичным для Windows поведением процессов. | (регистрирует `ctx.shell`) |
...
Листовой `cordis.yml` выбирает одну реализацию исполнителя и нужные ему обращённые к модели инструменты. Композиция с песочницей дополнительно выбирает провайдера `ctx.sandbox`; [пример ACP](../../examples/acp-agent/) показывает одну полную компоновку.
