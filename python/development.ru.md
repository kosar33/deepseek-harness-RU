...
Флаг `--skip-build` выручает, когда требуемые артефакты `lib/` уже существуют, а `--targets=node24-linux-x64,node24-linux-arm64,node24-macos-arm64` выбирает целевые платформы. Продукты попадают в `dist-exe/`, после чего скрипт синхронизирует выбранные носители в `python/sdk-runtime/`. Сборки под macOS дополнительно синхронизируют соответствующий spawn-helper, который требуется там `node-pty`.
...
`python/sdk/tests/test_bundled_runtime.py` упражняет доступные bundled-носители и пропускает носитель, чей артефакт ещё не собран. Общерепозиторная политика тестирования описана в [Тестировании](../docs/testing.ru.md).
...
Два сценария сравниваются с зафиксированным ожидаемым результатом под `scripts/snapshots/python-sdk-single-exe/`. Файл `minimal/model-visible.json` фиксирует для входящей в репозиторий минимальной композиции собранные системные промпты, объявляемые схемы инструментов и видимые модели сообщения, поэтому плагин, добавляющий нежданную секцию системного промпта или сообщение пользователя, заставит задание упасть; этот сценарий опускает динамический снапшот контекста рантайма, который та же композиция выдаёт на macOS и не выдаёт на Linux ([#2488](https://github.com/deepseek-harness/deepseek-harness/issues/2488)). Каталог `advanced/` фиксирует результат SDK и сохранённые журналы сессий. Перепрогоните отвечающий сценарий с `--update-snapshots` и просмотрите этот дифф до коммита.
